...
<div class="contMain"><!-- contMain starts--> 
  ...
  [{/if}]
   ...
   [{ if $oxcmp_basket->getDeliveryCosts() }]
      <div class="titleSec"><span class="titlelbl"><strong>[{ oxmultilang ident="PAYMENT_CHARGE" }] [{ $oxcmp_basket->getFDeliveryCosts() }] [{ $currency->sign}]</strong></span></div>
      [{ /if}]

  [{assign var="iPayError" value=$oView->getPaymentError() }]

  [{ if $iPayError == 1}]
    <br><div class="box errorbox error">[{ oxmultilang ident="PAYMENT_COMLETEALLFIELDS" }]</div>
  [{ elseif $iPayError == 2}]
    <br><div class="box errorbox error">[{ oxmultilang ident="PAYMENT_AUTHORIZATIONFAILED" }]</div>
  [{ elseif $iPayError == 4 }]
    <br><div class="box errorbox error">[{ oxmultilang ident="PAYMENT_UNAVAILABLESHIPPING" }]</div>
  [{ elseif $iPayError == 5 }]
    <br><div class="box errorbox error">[{ oxmultilang ident="PAYMENT_UNAVAILABLEPAYMENT" }]</div>
  [{ elseif $iPayError == 6 }]
    <br><div class="box errorbox error">[{ oxmultilang ident="PAYMENT_UNAVAILABLETSPROTECTION" }]</div>
  [{ elseif $iPayError > 6 }]
    <!--Add custom error message here-->
    <br><div class="box errorbox error">[{ oxmultilang ident="PAYMENT_UNAVAILABLEPAYMENT" }]</div>
  [{ elseif $iPayError == -1}]
    <br><div class="box errorbox error">[{ oxmultilang ident="PAYMENT_ERRUNAVAILABLEPAYMENT" }] "[{ $oView->getPaymentErrorText() }]").</div>
  [{ elseif $iPayError == -2}]
    <br><div class="box errorbox error">[{ oxmultilang ident="PAYMENT_NOSHIPPINGFOUND" }]</div>
  [{ elseif $iPayError == -3}]
    <br><div class="box errorbox error">[{ oxmultilang ident="PAYMENT_SELECTANOTHERPAYMENT" }]</div>
  [{/if}]
                  ...
                  [{ assign var="inptcounter" value="`$inptcounter+1`"}]
                    ...
                    [{if $sPaymentID == "oxidcashondel"}]
                        <div class="box info"> 
                            <table>
                             <tr>
                                  <td><input id="test_Payment_[{$sPaymentID}]" type="radio" name="paymentid" value="[{$sPaymentID}]" [{if $oView->getCheckedPaymentId() == $paymentmethod->oxpayments__oxid->value}]checked[{/if}]></td><td>
                                  </td>
                   ...
                   <div class="box info"> 
                    <table width="100%">
                        <tr>
                            <td width="5%"><input id="test_Payment_[{$sPaymentID}]" type="radio" name="paymentid" value="[{$sPaymentID}]" [{if $oView->getCheckedPaymentId() == $paymentmethod->oxpayments__oxid->value}]checked[{/if}]></td>
                      <td id="test_PaymentDesc_[{$smarty.foreach.PaymentSelect.iteration}]" colspan="2" width="95%"><label><b>[{ $paymentmethod->oxpayments__oxdesc->value}]</b></label></td>
                        </tr>
                    </table>
                   </div>
                ...
                <div class="footer_basket block clearfix">
                    <div class="btns">
	                    <div class="prev_step_on">
	                            <a href="[{$newurl}]" class="three-line">[{ oxmultilang ident="CHECKOUT_USER" }]</a>
                        </div>
                        <div class="next_step_on">
            <input id="test_UserNextStepBottom" name="userform" class="btn-new-ch" type="submit" value="CONTINUE">
